  ...
  template: `
    <div class="column">
      ...
      <div class="card" v-for="(card, cardIndex) in column.cards" :key="cardIndex">
        ...
        <p>Тип: {{ card.cardType }}</p> <!-- Added card type display -->
      ...
      <div class="add-card" v-if="isFirstColumn">
        ...
        <label for="newCardType">Тип карточки:</label> <!-- Added card type input -->
        <select id="newCardType" v-model="newCardType">
        ...
        <label v-if="newCardType === 'баг'" for="relatedTask">Связанный баг:</label>
        <select v-if="newCardType === 'баг'" id="relatedTask" v-model="relatedTask">
          <option value="">Выберите...</option>
          <option v-for="(card, index) in column.cards" :value="card.title" :key="index">{{ card.title }}</option>
        </select>
